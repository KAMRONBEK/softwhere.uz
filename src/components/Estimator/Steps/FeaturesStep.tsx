        ...
        {features.map(feature => (
          <div
            ...
            className={`border rounded-lg p-3 flex items-start cursor-pointer transition-all hover:shadow-md ${
              selectedFeatures.includes(feature.id) ? 'border-orange-500 bg-orange-50' : ''
            }`}
            onClick={() => onToggleFeature(feature.id)}
          >
            <div className='flex-1'>
              <div className='font-medium'>{(t as (k: string) => string)(feature.labelKey)}</div>
              <div className='text-xs text-gray-500'>+${((ESTIMATOR.FEATURE_HOURS[feature.id] ?? 0) * HOURLY_RATE).toLocaleString()}</div>
            </div>
