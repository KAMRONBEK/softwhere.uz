...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        if (!title || !slug || !content || !status || !locale) {
            return NextResponse.json({ error: 'Missing required fields (title, slug, content, status, locale)' }, { status: 400 });
        }
        ...
        if (slug !== existingPost.slug) {
            const slugCollision = await BlogPost.findOne({ slug: slug, locale: locale, _id: { $ne: id } }); // Check other posts in same locale
            if (slugCollision) {
