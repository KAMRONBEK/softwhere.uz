...
// Helper to generate an AI-powered estimate using Gemini 1.5
async function getAIEstimate(input: EstimatorInput): Promise<AIEstimateResponse | null> {
  if (!env.GOOGLE_API_KEY) {
    logger.warn('GOOGLE_API_KEY not configured, using fallback calculator', undefined, 'AI');
  ...
  try {
    ...
    try {
      ...
      const rawEstimate = JSON.parse(jsonStr);

      // Log the parsed estimate
      logger.info(`Parsed AI estimate: ${JSON.stringify(rawEstimate)}`, undefined, 'AI');
      ...
      const developmentCost = extractNumber(rawEstimate.developmentCost);
      const deadlineWeeks = extractNumber(rawEstimate.deadlineWeeks);
      const supportCost = extractNumber(rawEstimate.supportCost);

      // Final validation - if any value is invalid, return null
      ...
      return {
        ...
        reasoning: rawEstimate.reasoning || 'AI-powered estimate based on project parameters.',
      };
    } catch (parseError) {
    ...
    logger.error('AI estimation failed', error, 'AI');
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try to get an AI-powered estimate first
    const aiEstimate = await getAIEstimate(input);

    // If AI estimate is available and valid, return it
